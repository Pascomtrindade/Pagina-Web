...
export default function Donations(){
  ...
  const pixKey = parish.pixKey ?? ''            // garante string
  const pixBrCode = parish.pixBrCode ?? ''      // garante string
  const qrValue = pixBrCode || pixKey           // só mostra se tiver algum

  const copyPix = async () => {
    if (!pixKey) return                         // evita erro sem chave
    try {
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2 text-slate-700">
              <li className="flex flex-wrap items-center gap-2">
                ...
                <span className="font-mono break-all">{pixKey || '—'}</span>
                <button
                  onClick={copyPix}
                  disabled={!pixKey}
                  className="ml-2 rounded-lg border px-2 py-1 text-sm hover:bg-slate-50 disabled:opacity-50"
                >
            ...
            <div className="mt-4 grid place-items-center">
              {hasPix ? (
                ...
                <div className="rounded-2xl bg-slate-50 p-6 text-sm text-slate-500 ring-1 ring-slate-200">
                  Informe a <code>pixKey</code> ou <code>pixBrCode</code> no <code>config.ts</code> para gerar o QR Code.
                </div>
              )}
            </div>
